...
optotrak_data = dlmread('L3_B_Injured_FlexionExtension_3d.csv', ',', 5, 0);
...
lvm_data = dlmread('L3_B_Injured_FlexionExtension.lvm');
time_lvm = lvm_data(:, 8);  
...
ROM = max(beta_deg) - min(beta_deg);
NZ = max(beta_deg(abs(My_interp) < 0.1));

%% Plot Hysteresis Curve for β
figure;
hold on;
plot(My_interp(1,1500:2000), beta_deg(1500:2000,1), 'r', 'LineWidth', 1.5);
xlabel('Moment [Nm]');
ylabel('\beta Angle [°]');
title('Spinal Flexion-Extension');

yline(NZ, '--k', 'NZ', 'LabelHorizontalAlignment', 'left');
yline(max(beta_deg), '-.b', '+ROM', 'LabelHorizontalAlignment', 'left');
yline(min(beta_deg), '-.b', '-ROM', 'LabelHorizontalAlignment', 'left');
...
figure;
bar([NZ, ROM]);
xticklabels({'Flexion-Extension (β)'});
ylabel('Angle (°)');
title('Comparison of NZ and ROM for β');
grid on;
